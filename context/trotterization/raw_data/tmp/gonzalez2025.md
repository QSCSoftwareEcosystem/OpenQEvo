...
$$\mathcal{L}_{i,\sigma}(\cdot) = \sum_{l=1}^{3} \kappa_l \left( L_{i,\sigma}^{(l)}(\cdot) L_{i,\sigma}^{(l)\dagger} - \frac{1}{2} \{ L_{i,\sigma}^{(l)\dagger} L_{i,\sigma}^{(l)}, (\cdot) \} \right), \quad (2)$$
...
Tightness of Theorems 1 and 2. We can now ask if a version of Theorem 1 holds for bosonic systems i.e., is there a noise threshold  $\kappa_{\rm th}(U)$  dependent only on the non-Gaussian strength U and uniform in  $J,\Omega$  that guarantees a convex-Gaussian at all times? For dephasing
...
- [77] https://github.com/guillegg10/Separability\_Wigner-negativity, GitHub repository.
...
$$\mathcal{A}(\rho) = \sum_{i} A_i \rho B_i, \tag{S4}$$
...
The Hilbert space of m fermionic modes is described by the vacuum state  $|\text{vac}\rangle$  and the standard creation  $(a_i)$  and annihilation  $(a_i)$  operators, with  $i \in \{1, 2, \dots m\}$  labeling the fermionic mode. These satisfy the canonical anticommutation relations:
...
Similar to fermions, the Hilbert space of m bosonic modes will be described by a vacuum state  $|\text{vac}\rangle$  and the creation  $(a_i^{\dagger})$  and annihilation  $(a_i)$  operators, with  $i \in \{1, 2 \dots m\}$  labeling the bosonic mode. These satisfy the canonical commutation relations:

$$[a_i, a_{i'}] = 0 \text{ and } [a_i, a_{i'}^{\dagger}] = \delta_{i,i'}.$$
 (S11)

The Hilbert space of the bosonic model is the infinite-dimensional vector space given by  $\operatorname{span}\{\prod_i (a_i^{\dagger})^{\mu_i} | \operatorname{vac} \rangle : \mu_i \in \{0, 1, 2 \dots \}\}$ . It will be convenient to work with the 2m quadrature operators
...
$$\sum_{i':i'\neq i,\sigma'} \sum_{\alpha,\alpha'} |J_{i,\sigma;i',\sigma'}^{\alpha,\alpha'}(t)| \le J_C.$$
(S19)
...
While analyzing the bosonic model, it will be more convenient to express  $H_{\rm g}$  as a sum of particle number conserving and non-conserving terms via
...
$$\mathcal{L}_{n} = \sum_{l=1}^{3} \sum_{i,\sigma} \kappa_{l} \mathcal{D}_{L_{i,\sigma}^{(l)}}, \tag{S27}$$
...
| $\mathcal{G}_{v,u}(t)$                                                          | Eq. (S25)    | Multi-mode squeezing term between two bosonic modes      |
...
| m                                                                               | _            | Total number of modes $m = nL$                           |
...
= \exp \Big( -i U (\mathcal{N}_{1,l} \mathcal{N}_{2,l} - \mathcal{N}_{1,r} \mathcal{N}_{2,r}) + |U| (\mathcal{N}_{1,l} \mathcal{N}_{1,r} + \mathcal{N}_{2,l} \mathcal{N}_{2,r}) \Big), \tag{S33}$$
...
*Proof.* Consider the Trotterized state  $\sigma_T$  [Eq. (S29)]—note that  $\Phi^{\rm g}_{\tau\delta,(\tau-1)\delta}$  is a Gaussian channel and hence trivially preserves convex Gaussianity. We now obtain the condition under which  $\Phi^{\rm ng}_{\tau\delta,(\tau-1)\delta}$  also preserves convex Gaussianity using Lemma 4. We first perform the decomposition
...
**Assumption 1** (Uniform particle moment density). The initial state  $\rho(0)$  is a product state and  $\exists C_0, \alpha_0, \beta_0 > 0$  such that  $\forall v \text{ and } k \in \{1, 2, 3 ...\}$ 

$$\operatorname{Tr}(n_v^k \rho(0)) \le C_0^k k^{\alpha_0 k + \beta_0}.$$
...
$$\operatorname{Tr}(N^k \rho(0)) = \sum_{v_1, v_2 \dots v_k} \operatorname{Tr}(n_{v_1} n_{v_2} \dots n_{v_k} \rho(0)) \le \sum_{v_1, v_2 \dots v_k} \prod_{i=1}^k \operatorname{Tr}(n_{v_i}^k \rho(0))^{1/k} \le (C_0 m)^k k^{\alpha_0 k + \beta_0}, \tag{S39}$$
...
$$\sum_{q_{1}=1}^{k} \sum_{q_{2}=1}^{k-q_{1}} \cdots \sum_{q_{p}=1}^{k-\sum_{i=1}^{p-1} q_{i}} \frac{1}{q_{1}! q_{2}! \dots q_{p}!} f\left(\sum_{i=1}^{p} q_{i}\right) \stackrel{\text{(i)}}{=} \sum_{q=p}^{k} \sum_{q_{1}=1}^{q} \sum_{q_{2}=1}^{q-q_{1}} \cdots \sum_{q_{p}=1}^{q-\sum_{i=1}^{p-q} q_{i}} \frac{1}{q_{1}! q_{2}! \dots q_{p}!} f(q)$$
...
$$[a_v, N^k] = \frac{d^k}{dz^k} [a_v, e^{zN}] \Big|_{z=0} = a_v (N^k - (N-I)^k) = ((N+I)^k - N^k) a_v.$$
 (S49)
...
$$\langle \mathcal{L}_{\mathbf{n}}^{\dagger}(N^{k})\rangle_{t} = -(\kappa_{1} - \kappa_{2})k\langle N^{k}\rangle_{t} + \sum_{l=1}^{k} \left(\kappa_{1}(-1)^{l} \binom{k}{l+1} + \kappa_{2} \binom{k+1}{l+1}\right) \langle N^{k-l}\rangle_{t}$$
...
$$\sum_{l\geq 0} {k \choose 2l+1} {k-(2l+1) \choose q-(2l+1)} = \sum_{l\geq 0} \frac{k!}{(2l+1)!(k-q)!(q-(2l+1))!} = {k \choose q} \sum_{l\geq 0} {q \choose 2l+1} = 2^{q-1} {k \choose q}, \quad (S58)$$
...
$$\left| \langle [N^k, H_{\mathbf{g}}^{\mathrm{sq}}(t)] \rangle_t \right| \le 2\mathcal{G}k \langle N^k \rangle_t + mk\mathcal{G} \sum_{q \ge 1} \binom{k}{q} 2^q \langle N^{k-q} \rangle_t. \tag{S61}$$

Finally, we consider  $[N^k, H_{\rm g}^{\rm disp}(t)] = \sum_v \mathcal{D}_v(t)[N^k, a_v] - {\rm h.c.}$ —we begin by noting that, from Lemma 7,

$$[N^k, a_v] = ((N+I)^k - N^k) = \sum_{q>1} \binom{k}{q} N^{k-q} a_v,$$
 (S62)

and therefore

$$|\langle [N^{k}, H_{2}(t)] \rangle_{t}| \leq 2\Omega \sum_{v} \sum_{q \geq 1} {k \choose q} |\langle N^{k-q} a_{v} \rangle_{t}|$$
...
$$\leq \frac{\gamma}{2} k \langle N^{k} \rangle_{t} + \sum_{q \geq 1} \left( \frac{\gamma}{2} {k \choose q+1} + \frac{2m\Omega^{2}}{\gamma} {k \choose q} \rangle \langle N^{k-q} \rangle_{t}$$
...
$$\frac{d}{dt}\langle N^k \rangle_t \le -\frac{\gamma}{2} k \langle N^k \rangle_t + \lambda k m \sum_{q \ge 1} \left( \frac{k}{q} \right) \langle N^{k-q} \rangle_t, \tag{S65}$$
...
$$\mathcal{L}_{\leq d} = -i[H_{\leq d}, \cdot] + \sum_{l=1}^{3} \sum_{v} \kappa_l \mathcal{D}_{L_{v, \leq d}^{(l)}}, \tag{S67a}$$
...
$$\mathcal{P}_{\leq d}\mathcal{D}_{a_{v}}\mathcal{P}_{\leq d}(X) = \Pi_{v,\leq d}a_{v}\Pi_{v,\leq d}X\Pi_{v,\leq d}a_{v}^{\dagger}\Pi_{v,\leq d} - \frac{1}{2}\left(\Pi_{v,\leq d}n_{v}\Pi_{v,\leq d}X + X\Pi_{v,\leq d}n_{v}\Pi_{v,\leq d}\right)$$

$$= a_{v,\leq d}Xa_{v,\leq d}^{\dagger} - \frac{1}{2}\left(a_{v,\leq d}^{\dagger}a_{v,\leq d}X + Xa_{v,\leq d}^{\dagger}a_{v,\leq d}\right)$$

$$= \mathcal{D}_{a_{v,\leq d}}(X), \tag{S71a}$$

$$\mathcal{P}_{\leq d}\mathcal{D}_{a_{v}^{\dagger}}\mathcal{P}_{\leq d}(X) = \Pi_{v,\leq d}a_{v}^{\dagger}\Pi_{v,\leq d}X\Pi_{v,\leq d}a_{v}\Pi_{v,\leq d} - \frac{1}{2}\left(\Pi_{v,\leq d}a_{v}a_{v}^{\dagger}\Pi_{v,\leq d}X + X\Pi_{v,\leq d}a_{v}a_{v}^{\dagger}\Pi_{v,\leq d}\right)$$

$$\mathcal{P}_{\leq d} \mathcal{D}_{a_{v}^{\dagger}} \mathcal{P}_{\leq d}(X) = \Pi_{v, \leq d} a_{v}^{\dagger} \Pi_{v, \leq d} X \Pi_{v, \leq d} a_{v} \Pi_{v, \leq d} - \frac{1}{2} (\Pi_{v, \leq d} a_{v} a_{v}^{\dagger} \Pi_{v, \leq d} X + X \Pi_{v, \leq d} a_{v} a_{v}^{\dagger} \Pi_{v, \leq d})$$

$$= a_{v, \leq d}^{\dagger} X a_{v, \leq d} - \frac{1}{2} (a_{v, \leq d} a_{v, \leq d}^{\dagger} X + X a_{v, \leq d} a_{v, \leq d}^{\dagger}) - \frac{d+1}{2} (\Pi_{v, d} X + X \Pi_{v, d})$$

$$= \mathcal{D}_{a_{v, \leq d}^{\dagger}}(X) - \frac{d+1}{2} (\Pi_{v, d} X + X \Pi_{v, d}), \tag{S71b}$$
...
$$\begin{split} \|\mathcal{Q}_{\leq d}\rho(t)\|_{1} &= \|\Pi_{>d}\rho(t) + \Pi_{\leq d}\rho(t)\Pi_{>d}\|_{1} \\ &\leq \|\Pi_{>d}\rho(t)\|_{1} + \|\Pi_{\leq d}\rho(t)\Pi_{>d}\|_{1} \\ &\stackrel{\scriptscriptstyle{(1)}}{\leq} \sqrt{\mathrm{Tr}(\Pi_{>d}\rho(t))} + \sqrt{\mathrm{Tr}(\Pi_{\leq d}\rho(t))\mathrm{Tr}(\Pi_{>d}\rho(t))} \\ &\leq 2\sqrt{\mathrm{Tr}(\Pi_{\geq d}\rho(t))} \leq 2\sqrt{e} \left(\frac{d}{d_{0}m}\right)^{k_{0}/2} \exp\left(-\frac{1}{2}\left(\frac{d}{d_{0}m}\right)^{1/\alpha}\right), \end{split} \tag{S77}$$
...
$$\|\mathcal{P}_{\leq d}[H, \cdot]\|_{\diamond} \leq 2\|\Pi_{\leq d}H\| \text{ and } \|\mathcal{P}_{\leq d}\mathcal{D}_L\|_{\diamond} \leq \|\Pi_{\leq d}L\|^2 + \|\Pi_{\leq d}L^{\dagger}L\|.$$
 (S79)
...
where we have used the decomposition of  $H_g(t)$  in Eq. (S25). Furthermore,
...
Trotterization of the truncated model. We will perform a first-order Trotterization of the state  $\rho_{\leq d}(t) = \mathcal{T} \exp(\int_0^t \mathcal{L}_{\leq d}(s) ds) \mathcal{P}_{\leq d}(\rho(0))$ . We will split the Hamiltonian  $H_{\leq d}(t)$  into a sum of inter-site terms  $H_{\leq d}^{\text{C}}(t)$  and a sum of on-site terms  $H_{\leq d}^{\text{os}}(t)$ :

$$H_{\leq d}(t) = \underbrace{\sum_{i < j} \sum_{\sigma, \sigma'} h_{i, \sigma; j, \sigma'}^{C}(t)}_{H_{\leq d}^{C}(t)} + \underbrace{\sum_{i} \sum_{\sigma, \sigma'} h_{i; \sigma, \sigma'}^{os}(t)}_{H_{\leq d}^{os}(t)}, \tag{S84}$$

where, in  $h_{i,\sigma;j,\sigma'}^{\mathbf{C}}(t)$  we include all the terms that mediate an interaction between  $(i,\sigma)$  and  $(j,\sigma')$ :
...
where  $c^1_{i,\sigma;\leq d}=(a_{i,\sigma;\leq d}+a^\dagger_{i,\sigma;\leq d})/\sqrt{2},$   $c^2_{i,\sigma;\leq d}=(a_{i,\sigma;\leq d}-a^\dagger_{i,\sigma;\leq d})/\sqrt{2}i.$  In  $h^{\text{os}}_{i;\sigma,\sigma'}(t)$ , we include all the terms (Gaussian or non-Gaussian) that act between modes  $(i,\sigma)$  and  $(i,\sigma')$ :
...
where we will choose  $p_{i,\sigma;i',\sigma'}^{(l)}(t), q_{i,\sigma;i',\sigma'}^{(l)}(t) \ge 0$  later. For this decomposition of  $\mathcal{L}_{n,\le d}$  to be consistent, we must also have
...
$$\|\mathcal{L}_{i,\sigma;j,\sigma'}^{\mathbf{C}}(s)\|_{\diamond} \leq 2\|h_{i,\sigma;j,\sigma'}^{\mathbf{C}}(s)\| + 2\sum_{l=1}^{3} \kappa_{l} \left(p_{i,\sigma;j,\sigma'}^{(l)}(s)\|L_{i,\sigma,\leq d}^{(l)}\|^{2} + q_{i,\sigma;j,\sigma'}^{(l)}(s)\|L_{j,\sigma',\leq d}^{(l)}\|^{2}\right)$$

$$\stackrel{\text{(1)}}{\leq} \left(4|U_{i,\sigma;j,\sigma'}(s)| + 8\sum_{\alpha,\alpha'} |J_{i,\sigma;j,\sigma'}^{\alpha,\alpha'}(s)| + 2\sum_{l=1}^{3} \kappa_{l} (p_{i,\sigma;j,\sigma'}^{(l)}(s) + q_{i,\sigma;j,\sigma'}^{(l)}(s))\right) d^{2}, \tag{S90}$$

$$\|[\cdot, H^{\text{os}}(t)]\|_{\diamond} \le 2 \sum_{i, j, -l'} \|h_{i; \sigma, \sigma'}^{\text{os}}(t)\|$$

$$\stackrel{(2)}{\leq} \left( 2 \sum_{i,\sigma,\sigma'} |U_{i,\sigma;i,\sigma'}(s)| + 4 \sum_{i,\sigma,\sigma'} \sum_{\alpha,\alpha'} |J_{i,\sigma;i,\sigma'}^{\alpha,\alpha'}(s)| \right) d^2, \tag{S91}$$
...
$$\leq \underbrace{4(U_{C} + U_{os} + J_{C} + J_{os} + \kappa)md^{4}}_{\ell}.$$
(S92)
...
$$h_{\mathrm{g}, \leq d}(t) = \sum_{\alpha, \beta \in \{1, 2\}} g_{\alpha, \beta}(t) c_{1, \leq d}^{\alpha} c_{2, \leq d}^{\beta}, h_{\mathrm{ng}, \leq d}(t) = u(t) n_{1, \leq d} n_{2, \leq d}, \ L_{i}^{(1)} = a_{i, \leq d}, L_{i}^{(2)} = a_{i, \leq d}^{\dagger} \ \ and \ L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{i}^{(3)} = n_{i, \leq d}, L_{$$
...
$$\left\| \mathcal{M}_{t+\tau,t} - \mathcal{T} \exp\left( \int_t^{t+\tau} \mathcal{L}_{\leq d}(s) ds \right) \right\|_{\diamond} \leq 8d^4 \left( \sum_{\alpha,\beta} \int_t^{t+\tau} |g_{\alpha,\beta}(s)| \, ds + \int_t^{t+\tau} |u(s)| \, ds + \sum_{l=1}^3 \sum_{i \in I1, 2l} \int_t^{t+\tau} \kappa_i^{(l)}(s) ds \right)^2.$$
...
\frac{1}{2} \sum_{i\in\{1,2\}} \{K_{i}^{(1)} a_{i,\leq d}^{\dagger} a_{i,\leq d} + K_{i}^{(2)} a_{i,\leq d} a_{i,\leq d}^{\dagger}, \rho\} + \Delta_{t+\tau,t}(\rho), \tag{S95}$$
...
$$\tilde{\mathcal{R}}_{t+\tau,t}(\rho) = \mathbb{E}_y \big[ \tilde{R}_1(y) \tilde{R}_2(y) \rho \tilde{R}_2^{\dagger}(y) \tilde{R}_1^{\dagger}(y) \big], \tag{S98a}$$
...
$$\tilde{\mathcal{R}}_{t+\tau,t}(\rho) = \tilde{Q}_{1}\tilde{Q}_{2}\rho\tilde{Q}_{2}^{\dagger}\tilde{Q}_{1}^{\dagger} - iU(n_{1,\leq d}n_{2,\leq d}\rho\tilde{Q}_{1}^{\dagger}\tilde{Q}_{2}^{\dagger} - \tilde{Q}_{1}\tilde{Q}_{2}\rho n_{1,\leq d}n_{2,\leq d}) + \\
|U|\left(n_{1,\leq d}\tilde{Q}_{2}\rho\tilde{Q}_{2}^{\dagger}n_{1,\leq d} + \tilde{Q}_{1}n_{2,\leq d}\rho n_{2,\leq d}\tilde{Q}_{1}^{\dagger}\right) + |U|^{2}n_{1,\leq d}n_{2,\leq d}\rho n_{2,\leq d}n_{1,\leq d} \\
= \rho - i\left[\int_{t}^{t+\delta}h_{\mathrm{ng},\leq d}(s)ds,\rho\right] + |U|\left(n_{1,\leq d}\rho n_{1,\leq d} + n_{2,\leq d}\rho n_{2,\leq d}\right) - \frac{1}{2}K_{3}\{n_{1,\leq d} + n_{2,\leq d},\rho\} + \tilde{\Delta}_{t+\tau,t}(\rho), \tag{S99}$$
...
$$||E_{t+\tau,t}||_{\diamond} \le ||\Delta_{t+\tau,t}^{\mathcal{E}}||_{\diamond} + ||\Delta_{t+\tau,t}||_{\diamond} + ||\tilde{\Delta}_{t+\tau,t}||_{\diamond} \le 8d^{4}(G + U_{0} + K)^{2}, \tag{S107}$$
...
$$\tilde{\phi}_T = \sum_{\alpha} p_{\alpha} \left( \bigotimes_{i} \rho_i^{(\alpha)} \right), \tag{S116}$$
...
$$\sum_{k} |\operatorname{Tr}(L_{i,k}^{\dagger} A)|^{2} \ge \lambda_{0} \|A\|_{F}^{2}.$$
...
$$q_i^{\text{eff}}(t) = \frac{\kappa(t)}{2} \sum_k L_{i,k}^{\dagger} L_{i,k}, q^{\text{eff}} = q_1^{\text{eff}}(t) \otimes I + I \otimes q_2^{\text{eff}}(t) \text{ and } Q_i = \exp\left(-\int_t^{t+\tau} q_i^{\text{eff}}(t') dt'\right). \tag{S118}$$
...
$$\mathcal{R}_{t+\tau,t}(\rho) = (Q_1 \otimes Q_2)\rho(Q_1^{\dagger} \otimes Q_2^{\dagger}) - i\sum_{\alpha} S_{\alpha} ((O_{1,\alpha} \otimes O_{2,\alpha})\rho(Q_1^{\dagger} \otimes Q_2^{\dagger}) - (Q_1 \otimes Q_2)\rho(O_{1,\alpha} \otimes O_{2,\alpha})) +$$

$$\sum_{\alpha} S_{\alpha} ((O_{1,\alpha} \otimes Q_2)\rho(O_{1,\alpha} \otimes Q_2^{\dagger}) + (Q_1 \otimes O_{2,\alpha})\rho(Q_1^{\dagger} \otimes O_{2,\alpha})) +$$

$$\sum_{\alpha} S_{\alpha} S_{\alpha'} ((O_{1,\alpha} \otimes O_{2,\alpha})\rho(O_{1,\alpha'} \otimes O_{2,\alpha'}) + (O_{1,\alpha} \otimes O_{2,\alpha'})\rho(O_{1,\alpha} \otimes O_{2,\alpha'})), \tag{S120}$$
...
$$\Phi_{\mathcal{G}^{(i)}} = \sum_{j,j'=1}^{d} \left( K \sum_{k} L_{i,k} |j\rangle \langle j'| L_{i,k}^{\dagger} - \sum_{\alpha} S_{\alpha} O_{i,\alpha} |j\rangle \langle j'| O_{i,\alpha} \right) \otimes |j\rangle \langle j'|. \tag{S127}$$
...
$$\sum_{j>i} |s_{\alpha}^{i,j}(t)| + \sum_{j
(S131)$$
...
$$\frac{d}{dt}\rho(t) = \mathcal{L}\rho(t) = -i[H(t), \rho(t)] + \sum_{l=1}^{2} \sum_{v=1}^{m} \kappa_{l} \mathcal{D}_{L_{v}^{(l)}},$$
...
$$\|\tilde{H}_{dis}\| \le \frac{m}{2} \left(\kappa_1 d + \kappa_2 (d+1)\right) \le \frac{m}{2} \left(\kappa_1 + \kappa_2\right) d \le \frac{m\kappa}{2} (d+1). \tag{S136}$$
...
$$\sum_{l=1}^{2} \kappa_{l} \mathcal{D}_{L^{(l)},\alpha(t)}(\cdot) = \sum_{l=1}^{2} \kappa_{l} \mathcal{D}_{L^{(l)}} + \frac{i}{2} (\kappa_{1} - \kappa_{2}) \left[ i(\alpha(t)a^{\dagger} - \alpha^{*}(t)a), (\cdot) \right].$$
 (S146)
...
$$\frac{d}{dt}\rho(t) = -i[H(t), \rho(t)] + \sum_{l=1}^{2} \sum_{v=1}^{2} \kappa_{l} \mathcal{D}_{L_{v}^{(l)}}, \tag{S153}$$
...
$$\frac{d}{dt}\rho(t) = -i[J(a_1^{\dagger}a_2 + a_2^{\dagger}a_1), \rho(t)] + \sum_{i=1}^{4} \left(\kappa_1 \mathcal{D}_{a_i} + \kappa_2 \mathcal{D}_{a_j^{\dagger}}\right) \rho(t),$$
...
$$\operatorname{Tr}(O_A^{(+)}O_B^{(+)}\rho) = \operatorname{Tr}(O_AO_B\sigma). \tag{S163}$$
...
$$\operatorname{Tr}(O_A^{(+)}O_B^{(+)}\rho) = \int \operatorname{Tr}(O_A^{(+)}\rho_{A,x})\operatorname{Tr}(O_B^{(+)}\rho_{B,x})d\mu(x), \tag{S164}$$
...
$$\frac{d}{dt}\rho(t) = -i[J(a_2a_3 + a_3^{\dagger}a_2^{\dagger}), \rho(t)] + \sum_{j=1}^{4} (\kappa_1 \mathcal{D}_{a_j} + \kappa_2 \mathcal{D}_{a_j^{\dagger}})\rho(t).$$
...
$$\operatorname{Tr}(\rho(0)O_A^{(+)}O_B^{(+)}) = \frac{1}{2} \left\langle \psi_A^{(1)} | O_A^{(+)} | \psi_A^{(1)} \right\rangle \left\langle \psi_B^{(1)} | O_B^{(+)} | \psi_B^{(1)} \right\rangle + \frac{1}{2} \left\langle \psi_A^{(2)} | O_A^{(+)} | \psi_A^{(2)} \right\rangle \left\langle \psi_B^{(2)} | O_B^{(+)} | \psi_B^{(2)} \right\rangle, \tag{S167}$$
